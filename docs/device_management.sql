...
CREATE TABLE `stat_device_version` (
  ...
  `device_mac` varchar(32) DEFAULT NULL COMMENT '设备 wan mac',
  `version` varchar(64) DEFAULT NULL COMMENT '版本号',
  `version_update_on` datetime DEFAULT NULL COMMENT '版本更新时间',
  `online_days` int(11) DEFAULT NULL COMMENT '累计在线天数',
  `last_online_on` datetime DEFAULT NULL COMMENT '最后在线时间',
...
CREATE TABLE `device_upgrade_logs` (
  ...
  `device_id` int(11) DEFAULT NULL COMMENT '设备id',
  `device_mac` varchar(32) DEFAULT NULL COMMENT '设备 wan mac',
  `manual_flag` int(11) DEFAULT NULL COMMENT '检测标志:0->自动检测;1->手动检测',
  `upgrade_status` int(11) DEFAULT NULL COMMENT '升级操作:-1->升级失败;0->正常检测请求;1->设备开始升级;2->升级成功',
  `auto_upgrade_checked` int(11) DEFAULT NULL COMMENT '自动升级开关:0->关闭;1->打开',
  `local_upgrade_flag` int(11) DEFAULT NULL COMMENT '本地升级:0->远程升级;1->本地升级',
  `created_on` datetime DEFAULT NULL COMMENT '创建日期',
  PRIMARY KEY (`id`),
  KEY `device_id` (`device_id`),
  KEY `device_mac` (`device_mac`)
...
CREATE TABLE `device_type_model_relation` (
  ...
  `model_id` int(11) DEFAULT NULL COMMENT '设备型号id',
  `created_on` datetime DEFAULT NULL COMMENT '创建日期',
  `changed_on` datetime DEFAULT NULL COMMENT '更改日期',
  `created_by` int(11) DEFAULT NULL COMMENT '创建者',
  `changed_by` int(11) DEFAULT NULL COMMENT '更改者',
...
CREATE TABLE `firmwares` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '固件id',
  `model_id` int(11) DEFAULT NULL COMMENT '设备型号id',
  `version` varchar(64) DEFAULT NULL COMMENT '固件版本号',
  `show_version` varchar(64) DEFAULT NULL COMMENT '显示版本号',
  `show_upgrade_plan` int(11) DEFAULT NULL COMMENT '版本升级计划:0->隐藏;1->显示',
  `description` text DEFAULT NULL COMMENT '升级说明',
  `status` int(11) DEFAULT NULL COMMENT '上线状态:0->下线;1->上线',
  `upgrade_type` int(11) DEFAULT NULL COMMENT '升级类型:0->推荐升级;1->强制升级',
  `effect_on` datetime DEFAULT NULL COMMENT '生效时间',
  `upgrade_limit` int(11) DEFAULT NULL COMMENT '升级总量;0->不限制',
  `day_auto_upgrade_limit` int(11) DEFAULT NULL COMMENT '单日自动升级总量;0->不限制',
  `day_manual_upgrade_limit` int(11) DEFAULT NULL COMMENT '单日手动升级总量;0->不限制',
  `lowest_version` varchar(64) DEFAULT NULL COMMENT '允许升级的最低版本',
  `highest_version` varchar(64) DEFAULT NULL COMMENT '允许升级的最高版本',
  `upgrade_region_blacklist_flag` int(11) DEFAULT NULL COMMENT '升级地区限定:0->不限;1->允许升级地区;2->禁止升级地区',
  ...
  `file_name` varchar(128) DEFAULT NULL COMMENT '固件升级文件名',
  `download_url` varchar(255) DEFAULT NULL COMMENT '固件升级文件地址',
...
CREATE TABLE `device_activate_logs` (
  ...
  `device_id` int(11) DEFAULT NULL COMMENT '设备id',
  `device_mac` varchar(32) DEFAULT NULL COMMENT '设备 wan mac',
  `version` varchar(64) DEFAULT NULL COMMENT '激活版本',
  `ip_address` varchar(64) DEFAULT NULL COMMENT '激活IP',
  `operator` varchar(64) DEFAULT NULL COMMENT '激活运营商',
  `province` varchar(32) DEFAULT NULL COMMENT '激活省',
  `city` varchar(32) DEFAULT NULL COMMENT '激活市',
  `created_on` datetime DEFAULT NULL COMMENT '创建日期',
  PRIMARY KEY (`id`),
  KEY `device_id` (`device_id`),
  KEY `device_mac` (`device_mac`)
...
CREATE TABLE `firmware_upgrade_activity` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'id',
  `firmware_id` int(11) DEFAULT NULL COMMENT '固件id',
  `activity_limit_checked` int(11) DEFAULT NULL COMMENT '活跃度限制适用范围:0->全部设备;1->仅适用于勾选自动升级设备;2->仅适用于未勾选自动升级设备',
  `activation_start_on` datetime DEFAULT NULL COMMENT '激活起始时间',
  `activation_end_on` datetime DEFAULT NULL COMMENT '激活结束时间',
  `online_days` int(11) DEFAULT NULL COMMENT '在线天数限制',
  `manual_detection_checked` int(11) DEFAULT NULL COMMENT '手动检测次数符号:0->小于;1->大于',
  `manual_detection_times` int(11) DEFAULT NULL COMMENT '手动检测次数',
  `last_detection_on` datetime DEFAULT NULL COMMENT '最近检测时间',
  `local_upgrade_checked` int(11) DEFAULT NULL COMMENT '本地升级次数符号:0->小于;1->大于',
  `local_upgrade_times` int(11) DEFAULT NULL COMMENT '本地升级次数',
  `last_upgrade_on` datetime DEFAULT NULL COMMENT '最近升级时间',
  `created_on` datetime DEFAULT NULL COMMENT '创建日期',
  `changed_on` datetime DEFAULT NULL COMMENT '更改日期',
  `created_by` int(11) DEFAULT NULL COMMENT '创建者',
  `changed_by` int(11) DEFAULT NULL COMMENT '更改者',
...
CREATE TABLE `device_instances` (
  ...
  `wan_mac` varchar(32) DEFAULT NULL COMMENT 'WAN MAC 地址',
  `hard_version` varchar(64) DEFAULT NULL COMMENT '硬件版本',
  `soft_version` varchar(64) DEFAULT NULL COMMENT '固件版本',
  `auto_upgrade_checked` int(11) DEFAULT NULL COMMENT '自动升级开关:0->关闭;1->打开',
  `created_on` datetime DEFAULT NULL COMMENT '创建日期',
  PRIMARY KEY (`id`),
  KEY `mac` (`mac`)
...
--
-- Table structure for table `device_state_logs`
--
...
CREATE TABLE `device_state_logs` (
  ...
  `running` int(11) DEFAULT NULL COMMENT '运行时间',
  `cpu_used` int(11) DEFAULT NULL COMMENT 'CPU使用率',
  `mem_used` int(11) DEFAULT NULL COMMENT '内存使用率',
  `wifi_state` int(11) DEFAULT NULL COMMENT 'wifi状态:0->正常;其它值异常',
  `wan_lan_state` int(11) DEFAULT NULL COMMENT 'wan lan口状态:0->正常;其它值异常',
  `created_on` datetime DEFAULT NULL COMMENT '创建日期',
  PRIMARY KEY (`id`),
  KEY `device_id` (`device_id`),
  KEY `device_mac` (`device_mac`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='设备状态日志表';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `device_state_logs`
--

LOCK TABLES `device_state_logs` WRITE;
/*!40000 ALTER TABLE `device_state_logs` DISABLE KEYS */;
/*!40000 ALTER TABLE `device_state_logs` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `device_upgrade_attributes` (
  ...
  `auto_upgrade_start` varchar(32) DEFAULT NULL COMMENT '自动升级开始时间',
  `auto_upgrade_end` varchar(32) DEFAULT NULL COMMENT '自动升级结束时间',
  `auto_upgrade_condition` varchar(32) DEFAULT NULL COMMENT '自动升级条件:LowFlow(流量小于100k时执行),NoWirelessConnected(没有无线连接时执行)',
  `created_on` datetime DEFAULT NULL COMMENT '创建日期',
  `changed_on` datetime DEFAULT NULL COMMENT '更改日期',
  `created_by` int(11) DEFAULT NULL COMMENT '创建者',
  `changed_by` int(11) DEFAULT NULL COMMENT '更改者',
...
-- Dump completed on 2021-08-04 17:59:35

package com.sim.wifi.generator.genenrator;
...
//        System.out.println("-->projectPath = " + projectPath);
        ...
        String parentModuleName = scanner("父模块名");
        String moduleName = scanner("模块名");
        StringBuilder projectPath = new StringBuilder(System.getProperty("user.dir"));
        if ((parentModuleName != null) && (parentModuleName.length() > 0)) {
            projectPath.append("\\").append(parentModuleName);
        }
        projectPath.append("\\").append(moduleName);
        System.out.println("-->projectPath = " + projectPath);
        String[] tableNames = scanner("表名，多个英文逗号分割").split(",");
        // 代码生成器
        AutoGenerator autoGenerator = new AutoGenerator();
        autoGenerator.setGlobalConfig(initGlobalConfig(projectPath.toString()));
        autoGenerator.setDataSource(initDataSourceConfig());
        autoGenerator.setPackageInfo(initPackageConfig(moduleName));
        autoGenerator.setCfg(initInjectionConfig(projectPath.toString(), moduleName));
...
//        throw new MybatisPlusException("请输入正确的" + tip + "！");
    ...
    private static GlobalConfig initGlobalConfig(String projectPath) {
        ...
        globalConfig.setAuthor("wifi-upgrade");
...
//        packageConfig.setModuleName(moduleName);
        packageConfig.setParent(props.getStr("package.base") + "." + moduleName);
